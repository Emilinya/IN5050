    ...
    struct data_segment **data_segment, struct c63_common *cm)
{
    ...
    int mb_count = cm->mb_rows * cm->mb_cols;
    size_t mb_size = sizeof(struct macroblock);
    int data_segment_size = (3 * sizeof(uint8_t) + sizeof(int16_t)) * imsize;
    data_segment_size += (mb_count + mb_count / 2) * mb_size;

    SCICreateSegment(
        *sd, localDataSegment, GET_SEGMENTID(3), data_segment_size,
        NO_CALLBACK, NULL, NO_FLAGS, &error);
    ERR_CHECK(error, "SCICreateSegment");
    SCIPrepareSegment(*localDataSegment, LOCAL_ADAPTER_NUMBER, NO_FLAGS, &error);
    ERR_CHECK(error, "SCIPrepareSegment");

    struct data_segment *data_ptr = malloc(sizeof(struct data_segment));
    data_ptr->reference_recons = malloc(sizeof(yuv_t));
    data_ptr->currenct_recons = malloc(sizeof(yuv_t));
    data_ptr->predicted = malloc(sizeof(yuv_t));
    data_ptr->residuals = malloc(sizeof(dct_t));

    sci_map_t dm;
    sci_local_segment_t *lds = localDataSegment;

    int offset = 0;
    map_yuv(data_ptr->reference_recons, 0, lds, NULL, &dm, NULL, &offset, ysize, usize, vsize, &error);
    map_yuv(data_ptr->currenct_recons, 0, lds, NULL, &dm, NULL, &offset, ysize, usize, vsize, &error);
    map_yuv(data_ptr->predicted, 0, lds, NULL, &dm, NULL, &offset, ysize, usize, vsize, &error);
    map_dct(data_ptr->residuals, 0, lds, NULL, &dm, NULL, &offset, ysize, usize, vsize, &error);

    data_ptr->mbs[Y_COMPONENT] = mapper(0, lds, NULL, &dm, NULL, &offset, mb_count * mb_size, &error);
    data_ptr->mbs[U_COMPONENT] = mapper(0, lds, NULL, &dm, NULL, &offset, mb_count / 4 * mb_size, &error);
    data_ptr->mbs[V_COMPONENT] = mapper(0, lds, NULL, &dm, NULL, &offset, mb_count / 4 * mb_size, &error);

    (*data_segment) = data_ptr;
